...
package net.dreamcraftnetwork.feedingrebalanced.world.structure;
...
public class StructureFarmersShack extends ElementsFeedingRebalanced.ModElement {
	public StructureFarmersShack(ElementsFeedingRebalanced instance) {
		super(instance, 31);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if ((random.nextInt(1000000) + 1) <= 10000) {
			...
			for (int a = 0; a < count; a++) {
				...
				require = Blocks.SANDSTONE.getStateFromMeta(1);
				try {
					if ((blockAt.getBlock() == require.getBlock())
							&& (blockAt.getBlock().getMetaFromState(blockAt) == require.getBlock().getMetaFromState(require)))
						blockCriteria = true;
				...
				require = Blocks.SANDSTONE.getStateFromMeta(2);
				try {
					if ((blockAt.getBlock() == require.getBlock())
							&& (blockAt.getBlock().getMetaFromState(blockAt) == require.getBlock().getMetaFromState(require)))
						blockCriteria = true;
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("feedingrebalanced", "farmers_shack_with_loot_using_farmers_storage_v2"));
				if (template == null)
					return;
